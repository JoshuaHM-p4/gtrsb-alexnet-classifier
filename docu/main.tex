...
For future research, we recommend training the convolutional feature layers using a gradual unfreezing schedule, which could allow the network to learn low-level shapes specific to traffic regulations. Applying advanced data augmentation methods, such as synthetic generation of blurred or occluded signs, would likely improve robustness under sub-optimal outdoor conditions. Additionally, incorporating multi-national traffic sign datasets from other countries (e.g., US speed limits or Chinese warning symbols) would build a more globally generalized and universal approach to classification. To transition this pipeline into production, future work should extend the model beyond single-image classification to real-time object detection frameworks (e.g., YOLO or Faster R-CNN) to facilitate simultaneous localization and categorization of multiple signs. Lastly, deploying the trained model on edge computing hardware would provide valuable insights into its inference latency and energy efficiency in real-time driver assistance systems.
